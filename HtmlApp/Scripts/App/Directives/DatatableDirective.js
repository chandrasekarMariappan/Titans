    ...
    return {
        ...
        link: function (scope, element, attrs, controller) {
            ...
            element.on('click', 'tbody tr td', function (event, args) {
                if (scope.rowClickEnabled) {

                    $($(this).parent()[0]).toggleClass('rowSelected');
                    if ($($(this).parent()[0]).hasClass('rowSelected')) {
                        scope.selectedItems[$($(this).parent()[0]).attr('id')] = $($(this).parent()[0]).attr('id');
                        $($(this).children()[0]).removeClass('glyphicon-unchecked').addClass('glyphicon-check');
                    }
                    else {
                        delete scope.selectedItems[$($(this).parent()[0]).attr('id')];
                        $($(this).children()[0]).removeClass('glyphicon-check').addClass('glyphicon-unchecked');
                    }

                    scope.selectedCount = Object.keys(scope.selectedItems).length;
